...
void invoke_fmt(const uint8_t* Data, std::size_t Size) {
  ...
  const auto prepared = fmt::prepare<Item>(fmtstring);
  prepared.format_to(message, item);

  fmt::memory_buffer message2;
  fmt::format_to(message2, fmtstring, item);
  assert(message.size()==message2.size());
  for(std::size_t i=0; i<message.size(); ++i) {
  assert(message[i]==message2[i]);
  }
#endif
